...
impl FeeConfiguration {
    ...
    pub fn new_l1(bundle_fee: u32, swap_fee: u32, protocol_fee: u32) -> Self {
        FeeConfiguration::L1(L1FeeConfiguration { bundle_fee, swap_fee, protocol_fee })
    }
    ...
    pub fn swap_fee(&self) -> u32 {
        match self {
            ...
            // L2 returns 0 for swap fee in beforeSwap - all fees are applied in afterSwap
            FeeConfiguration::L2(_) => 0
        }
    }
    ...
    pub fn protocol_fee(&self) -> u32 {
        match self {
            ...
            FeeConfiguration::L2(cfg) => cfg.creator_swap_fee_e6 + cfg.protocol_swap_fee_e6
        }
    }

    pub fn bundle_fee(&self) -> Option<u32> {
        match self {
            FeeConfiguration::L1(cfg) => Some(cfg.bundle_fee),
            FeeConfiguration::L2(_) => None
        }
    }
    ...
    pub fn fee(&self, bundle: bool) -> u32 {
        match self {
            ...
            // L2 doesn't have bundle mode - all fees are applied via protocol_fee()
            FeeConfiguration::L2(_) => self.swap_fee() + self.protocol_fee()
        }
    }
